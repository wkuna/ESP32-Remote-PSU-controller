...
void ESP32AnalogRead::checkEfuse(void)
{
...
uint32_t ESP32AnalogRead::readMiliVolts()
{
    ...
    for (int i = 0; i < NO_OF_SAMPLES; i++)
    {
        ...
        if (unit == ADC_UNIT_1)
        {
            raw_reading = adc1_get_raw(adc1_channel);
        }
        else
        {
            adc2_get_raw(adc2_channel, ADC_WIDTH_12Bit, &raw_reading);
        }
        adc_reading += raw_reading;
    }
    adc_reading /= NO_OF_SAMPLES;

    voltage = esp_adc_cal_raw_to_voltage(adc_reading, &characteristics);
    return voltage;
}